...
AsteroidGraphics::AsteroidGraphics() : indices_count(0) {}

void AsteroidGraphics::Apply(ID3D11DeviceContext* deviceContext)
{
...
void AsteroidGraphics::ApplyTransform(ID3D11DeviceContext* deviceContext, const AsteroidGraphics::WorldTransform &buf)
{
	deviceContext->UpdateSubresource(dataBuffer, 0, NULL, &buf, 0, 0);
}

void AsteroidGraphics::Render(ID3D11DeviceContext* deviceContext)
{
	deviceContext->DrawIndexed(indices_count, 0, 0);
}

void AsteroidGraphics::Load(ID3D11Device* device, ShaderCollection coll)
{
	shader = coll.GetShader(L"texture");

	Mesh mesh = loadMeshFromFile_ply("models/asteroid.ply");
	...
	HRESULT hr = D3DX11CreateShaderResourceViewFromFile(device, L"textures/asteroid.jpg", NULL, NULL, &resourceView, NULL);
